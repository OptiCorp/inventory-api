...
public class SizeController(
    ...
    ISizeCreateValidator createValidator,
    ISizeUpdateValidator updateValidator)
    : ControllerBase
{
    [HttpGet]
    ...
    public async Task<ActionResult<IEnumerable<Size>>> GetAllSizes()
    {
        try
        {
            return Ok(await sizeService.GetAllSizesAsync());
    ...
    {
        var validationResult = await createValidator.ValidateAsync(sizeCreate);
        ...
        try
        {
            var sizeId = await sizeService.CreateSizeAsync(sizeCreate);
            if (sizeId == null)
            {
    ...
    {
        var validationResult = await updateValidator.ValidateAsync(sizeUpdate);
        if (!validationResult.IsValid)
        {
            var modelStateDictionary = new ModelStateDictionary();
            foreach (var failure in validationResult.Errors)
        ...
        try
        {
            ...
            await sizeService.UpdateSizeAsync(sizeUpdate);
