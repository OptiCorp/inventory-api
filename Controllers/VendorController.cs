...
public class VendorController(
    ...
    IVendorCreateValidator createValidator,
    IVendorUpdateValidator updateValidator)
    : ControllerBase
{
    [HttpGet]
    ...
    public async Task<ActionResult<IEnumerable<Vendor>>> GetAllVendors()
    {
        try
        {
            return Ok(await vendorService.GetAllVendorsAsync());
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult<IEnumerable<Vendor>>> GetVendorBySearchString(string searchString)
    {
        try
        {
            return Ok(await vendorService.GetAllVendorsBySearchStringAsync(searchString));
        }
        catch (Exception e)
        {
            return BadRequest($"Something went wrong: {e.Message}");
    ...
    {
        var validationResult = await createValidator.ValidateAsync(vendorCreate);
        ...
        try
        {
            var vendorId = await vendorService.CreateVendorAsync(vendorCreate);
            if (vendorId == null)
            {
    ...
    {
        var validationResult = await updateValidator.ValidateAsync(vendorUpdate);
        if (!validationResult.IsValid)
        {
            var modelStateDictionary = new ModelStateDictionary();
            foreach (var failure in validationResult.Errors)
        ...
        try
        {
            ...
            await vendorService.UpdateVendorAsync(vendorUpdate);
